...
// i-ая проба:
// g(k, i)= (h(k) + g(k, 1) * i) >> (64 - M)
// a, b - случайные числа, M - число битов в размере
...
#define INITIAL_CAPACITY 8
	...
	inline uint64_t FirstHash(const std::string &s) {
		...
		//std::cout << h << " " << ((a * h + b) >> (64 - M)) << std::endl;
	...
	uint64_t secondHashValue;
	inline uint64_t SecondHash(uint64_t prevHash, unsigned int i) {
		// g(k, i)= (h(k) + g(k, 1) * i) >> (64 - M)
		if (i == 1) {
			// Насильно делаем нечётным для взаимной простоты с размером
			secondHashValue = ((d * prevHash + e) >> (64 - M)) | 1;
		}
		// На следующих после 1 итерациях просто прибавляем вычисленное
		// на первой итерации значение (и берём результат по модулю)
		return (prevHash + secondHashValue) & mask;
	}
	...
	unsigned int find_internal(const std::string &s, bool forInsert = false) {
		...
		uint64_t hash = FirstHash(s);
		unsigned int place = (unsigned int) (hash >> (64 - M));
		...
		for (unsigned int i = 1; !v[place].empty() || (!forInsert && deleted[place]); i++) {
			...
			// Генерируем вторичный хеш c i-й итерацией
			hash = SecondHash(hash, i);
			place = (unsigned int) hash;
		}
